...
abstract class WalletRepoAbs {
  Future<Either<KFailure, String>> get_wallet_amount();
...
class WalletRepoImp implements WalletRepoAbs {
  ...
  @override
  Future<Either<KFailure, String>> get_wallet_amount() async {
    Future<Response<dynamic>> func = Di.dioClient.get(KEndPoints.getWalletAmount);
    final result = await ApiClientHelper.responseOrFailure(func: func);
    return result.fold(
      (l) => left(l),
      (r) => right(r['data']['amount']),
    );
  }
  ...
  Future<Either<KFailure, AccountData>> get_account_history() async {
    Future<Response<dynamic>> func = Di.dioClient.get(KEndPoints.getAccountHistory);
    final result = await ApiClientHelper.responseOrFailure(func: func);
    return result.fold(
          (l) => left(l),
